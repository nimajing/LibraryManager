...
// executions.
bool EatCodePoint(const std::string &input, size_t start, size_t *end,
                  uint32_t *ucs) {
  if (start >= input.size()) {
    *end = start + 1;
    return false;
...
// |end| to represent the beginning of the next byte to eat for consecutive
// executions.
bool EatCodePoint(const std::wstring &input, size_t start, size_t *end,
                  uint32_t *ucs) {
  if (start >= input.size()) {
    *end = start + 1;
    return false;
...
bool IsCombining(uint32_t ucs) { return Bisearch(ucs, g_extend_characters); }
...
int wchar_width(wchar_t ucs) { return codepoint_width(uint32_t(ucs)); }
...
Utf8ToWordBreakProperty(const std::string &input) {
